...
ArcFaceEngine::ArcFaceEngine()
{
     ...
     int fd_workmem_size = 40 * 1024 * 1024;
     mFDWorkMem = new unsigned char[fd_workmem_size];
     ...
     int ret = AFD_FSDK_InitialFaceEngine((MPChar)APPID, (MPChar)FD_SDKKEY, mFDWorkMem, fd_workmem_size, &hFDEngine, AFD_FSDK_OPF_0_HIGHER_EXT, 16, MAX_FD_FACE);
     if (ret != 0) {
         qDebug() <<"fail to AFD_FSDK_InitialFaceEngine():"<<ret;
         exit(0);
     }


     ret = AFT_FSDK_InitialFaceEngine((MPChar)APPID, (MPChar)FT_SDKKEY, mFTWorkMem, ft_workmem_size, &hFTEngine, AFT_FSDK_OPF_0_HIGHER_EXT, 16, MAX_FT_FACE);
     if(ret != 0){
...
int ArcFaceEngine::getAllFile(string dir, string ext, vector<string> &files)
{
    DIR *pDir;
    if (dir.empty())
        pDir = opendir(".");
    ...
    if (nullptr == pDir)
    {
        qDebug() << "Can not open dir: " << dir.c_str();
        return -1;
    }
...
int ArcFaceEngine::loadFaceDB()
{
    //exit(0);
    string faceimgdir = "faceDB";

    vector<string> allFiles;

    getAllFile(faceimgdir, "dat", allFiles);
    ...
    for (iter = allFiles.begin(); iter != allFiles.end(); iter++)
    {
        ...
        pbuf = ifs.rdbuf();
        flen = pbuf->pubseekoff(0,ios::end, ios::in);

        qDebug() << QString::fromStdString(*iter) << " size: " << flen;
        pbuf->pubseekpos (0, ios::in);
        faceinfo.featureSize = flen;
        faceinfo.pFeature = new MByte[faceinfo.featureSize];
        pbuf->sgetn((char*)faceinfo.pFeature, faceinfo.featureSize);
        ifs.close();


        faceinfo.name =QString::fromStdString((*iter).substr(0, (*iter).length()-4));
...
int ArcFaceEngine::detectFace(unsigned char *frameData, int frameWidth, int frameHeight, int frameFormat, int *faceCount, LPAFT_FSDK_FACERES *ppFaceResult)
{
    int frameSize = 0;
    ASVLOFFSCREEN inputImg = { 0 };
    inputImg.u32PixelArrayFormat = frameFormat;
    ...
    int ret = AFT_FSDK_FaceFeatureDetect(hFTEngine, &inputImg, ppFaceResult);
    if (ret != 0)
    {
        *faceCount = 0;
    }
    ...
    {
        *faceCount = (*ppFaceResult)->nFace;
    }
    return 0;
}

int ArcFaceEngine::detectFace_FD(unsigned char *frameData, int frameWidth, int frameHeight, int frameFormat, int &faceCount, LPAFD_FSDK_FACERES &pFaceResult)
{
    int imgSize = 0;
    ASVLOFFSCREEN inputImg = {0};
    inputImg.u32PixelArrayFormat = frameFormat;
...
int ArcFaceEngine::recognitionFace(unsigned char *frameData,int frameWidth,int frameHeight,int frameFormat,MRECT *pRect,int faceOrient,int *faceIndex,float *pfScore){
    int facenum = mRegisterFaces.count();
    *faceIndex = -1;
    if(facenum<=0){
        return -1;
    ...
    *pfScore = 0.0f;
    ...
    if(((*faceIndex)>=0)&&((*pfScore)>mThreshold)){
